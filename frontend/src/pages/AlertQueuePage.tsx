...
export function AlertQueuePage() {
  ...
  return <section className="alert-queue-page">
    <div className="queue-toolbar"><div><p className="eyebrow">Analyst operations</p><h2>Live alert queue</h2><p>Prioritized by severity and refreshed every five seconds.</p></div><div className="refresh-block">{refreshing && <InlineSpinner label="Refreshing" />}<span>Last updated<br /><strong>{lastUpdated ? lastUpdated.toLocaleTimeString() : "Not yet"}</strong></span><button className="secondary-button" onClick={() => void load(true)} disabled={refreshing}>Refresh now</button></div></div>

    <div className="queue-summary" aria-label="Current result summary">{Object.entries(summary).map(([label, count]) => <article key={label}><span>{label}</span><strong>{count}</strong></article>)}</div>

    <form className="alert-filters" onSubmit={submitSearch}>
...
function AlertTable({ alerts, total }: { alerts: Alert[]; total: number }) {
  return <div className="alert-table-card"><div className="table-caption"><span>Showing {alerts.length} of {total} returned alerts</span><span>Current results</span></div><div className="alert-table-scroll"><table className="alert-table"><thead><tr><th>Priority</th><th>Status</th><th>Risk</th><th>Primary reason</th><th>Transaction</th><th>Assigned analyst</th><th>Last activity</th><th aria-label="Actions" /></tr></thead><tbody>{alerts.map((alert) => <tr key={alert.id}><td data-label="Priority"><PriorityBadge priority={alert.priority} /></td><td data-label="Status"><StatusBadge status={alert.status} /></td><td data-label="Risk"><RiskBadge level={riskLevel(alert.riskScore)} /><strong className="risk-number">{alert.riskScore}</strong></td><td data-label="Reason"><span className="reason-text" title={alert.primaryReason}>{alert.primaryReason}</span></td><td data-label="Transaction"><code>{shortId(alert.transactionId)}</code></td><td data-label="Assigned"><span className={alert.assignedTo ? "assignee" : "assignee unassigned"}>{alert.assignedTo ? shortId(alert.assignedTo) : "Unassigned"}</span></td><td data-label="Last activity"><time dateTime={lastActivity(alert)}>{formatDate(lastActivity(alert))}</time></td><td><Link className="view-alert-link" to={`/alerts/${alert.id}`} aria-label={`Open alert ${alert.id}`}>View alert →</Link></td></tr>)}</tbody></table></div></div>;
}
