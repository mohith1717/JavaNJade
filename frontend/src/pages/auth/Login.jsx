...
import {
  ...
  Dialog, DialogContent, DialogTitle, Alert, Snackbar, Stack, Checkbox, FormControlLabel,
} from '@mui/material';
...
const ROLE_ROUTE_MAP = {
  [ROLES.ADMIN]: '/',
  [ROLES.FRAUD_ANALYST]: '/transactions',
  [ROLES.RISK_ANALYST]: '/analytics',
};
...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      otherStore.removeItem('authToken');
      tokenStore.setItem('authToken', token);
      setUser(user, rememberMe);
      setSnackbar({
        open: true,
        message: `Welcome, ${user?.name ?? user?.username ?? 'user'}. Authentication successful.`,
        severity: 'success',
      });
  ...
  return (
    <Box component="main" aria-label="Login page" sx={{
      minHeight: '100vh',
      display: 'flex',
      alignItems: 'center',
      ...
      '&::after': {
        ...
        inset: 0,
        zIndex: 0,
        background: 'linear-gradient(110deg, transparent 0%, rgba(0,212,255,0.08) 45%, transparent 100%)',
      ...
      <Box
        ...
        sx={{ position: 'absolute', width: 400, height: 400, borderRadius: '50%', bgcolor: 'rgba(0,212,255,0.04)', filter: 'blur(80px)', top: '-10%', right: '-5%', zIndex: 0 }}
      />
      <Box
        component={motion.div}
        animate={{ x: [0, 26, 0], y: [0, -16, 0] }}
        transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut' }}
        sx={{ position: 'absolute', width: 320, height: 320, borderRadius: '50%', bgcolor: 'rgba(123,47,255,0.06)', filter: 'blur(80px)', bottom: '5%', left: '-5%', zIndex: 0 }}
      />

      {/* Login card */}
      <Box component={motion.div} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38, ease: 'easeOut' }} sx={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 420, px: 2 }}>
        ...
        <Stack alignItems="center" spacing={1.5} sx={{ mb: 3 }}>
          ...
          <Box textAlign="center">
            <Typography sx={{ fontSize: '1.2rem', fontWeight: 800, color: '#E8EDF5', letterSpacing: '-0.3px', display: 'inline-flex', alignItems: 'center' }}>
              JavaNJade
              <Box
                component="span"
                aria-hidden="true"
        ...
        <Card sx={{
          ...
          <CardContent sx={{ p: 3.5 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              Sign in to your enterprise account with secure role-based access
            </Typography>
            ...
            <Stack spacing={2}>
              <TextField
                ...
                helperText="Enter your enterprise username"
                InputProps={{
                  endAdornment: focusedField === 'username' ? (
                    <InputAdornment position="end">
                      <Box component="span" sx={{ width: 2, height: 18, bgcolor: '#00D4FF', animation: 'blinkCursor 1s steps(1) infinite' }} />
                    </InputAdornment>
                  ) : null,
                }}
                sx={inputFocusStyles}
              ...
              <TextField
                ...
                helperText="Password is hidden by default for privacy"
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end">
                      <IconButton aria-label={showPass ? 'Hide password' : 'Show password'} onClick={() => setShowPass((p) => !p)} edge="end" size="small">
                        {showPass
            ...
            <Box sx={{ bgcolor: 'rgba(0,212,255,0.05)', borderRadius: 2, p: 1.5, border: '1px solid rgba(0,212,255,0.1)' }}>
              ...
              <Typography variant="caption" color="text.secondary" display="block">
                Risk: <strong style={{ color: '#00FF88' }}>risk</strong> / <strong style={{ color: '#00FF88' }}>risk123</strong>
              </Typography>
            </Box>
          </CardContent>
        </Card>
