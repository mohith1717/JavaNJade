...
class RuleManagementApiIntegrationTests {
    ...
    @Test
    void statusPatchWithMissingEnabledReturnsValidationEnvelope()
            throws Exception {
        String ruleId = createAndReadId(HIGH_AMOUNT_RULE);
        ...
        mockMvc.perform(patch("/api/rules/{id}/status", ruleId)
                ...
                .andExpect(jsonPath("$.path")
                        .value("/api/rules/" + ruleId + "/status"));
    }

    @Test
    void invalidRuleTypeFilterReturnsBadRequestEnvelope() throws Exception {
        mockMvc.perform(get("/api/rules")
                        .queryParam("type", "NOT_A_RULE_TYPE"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.status").value(400))
                .andExpect(jsonPath("$.error").value("Bad Request"))
                .andExpect(jsonPath("$.message").value(
